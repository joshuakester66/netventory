...
IP_SELECT_SNMP="SELECT ip FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE (snmp_enabled='1' AND (ip LIKE '10.%' OR ip LIKE '172.16.%' OR ip LIKE '192.168.%')) AND ((ping_check > (NOW() - INTERVAL 7 DAY) AND updated < (NOW() - INTERVAL 1 DAY)) OR added > (NOW() - INTERVAL 1 DAY));"
IP_SELECT_NO_SNMP="SELECT ip FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE (snmp_enabled='1' AND (ip LIKE '10.%' OR ip LIKE '172.16.%' OR ip LIKE '192.168.%')) AND ((ping_check > (NOW() - INTERVAL 7 DAY) AND updated < (NOW() - INTERVAL 1 DAY)) OR added > (NOW() - INTERVAL 1 DAY));"
...
# IPS=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT ip FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE snmp_enabled='1' AND ping_check > (NOW() - INTERVAL 7 DAY) AND (ip LIKE '10.%' OR ip LIKE '172.16.%' OR ip LIKE '192.168.%');"`
# IPS=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT ip FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE (snmp_enabled='1' AND (ip LIKE '10.%' OR ip LIKE '172.16.%' OR ip LIKE '192.168.%')) AND ((ping_check > (NOW() - INTERVAL 7 DAY) AND updated < (NOW() - INTERVAL 1 DAY)) OR added > (NOW() - INTERVAL 1 DAY));"`
...
# INTERFACE_UPDATE=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT updated FROM $LOCAL_DATABASE.$INTERFACE_TABLE WHERE device_id='$DEVICE_ID' AND updated < (NOW() - INTERVAL 1 DAY) ORDER BY updated DESC LIMIT 1;"`
...
EOF
		...
		mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $LOCAL_DATABASE << EOF
UPDATE $DEVICE_TABLE SET description='$SYSDESCR',hostname='$SYSNAME',sysname='$SYSNAME',syslocation='$SYSLOCATION',model='$MODEL',firmware_p='$FIRMWARE_P',firmware_s='$FIRMWARE_S',serial='$SERIAL',manufacturer='$MANUFACTURER',rom='$ROM',updated=NOW(),dns_name='$DNS_NAME',mac='$MAC',oui='$OUI' WHERE id='$DEVICE_ID';
EOF
fi
...
EOF
		...
		if [[ -n "$INTERFACE" ]]
			then
				INTERFACE_ID=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT id FROM $LOCAL_DATABASE.$INTERFACE_TABLE WHERE device_id='$DEVICE_ID' AND port='$INTERFACE';"`
		fi
		if [[ -z "$INTERFACE_ID" ]]
			then
				INTERFACE_ID="0"
		...
		ALT_IP_ID=
		ALT_IP_ID=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT id FROM $LOCAL_DATABASE.$ADDRESS_TABLE WHERE ip='$ALT_IP' AND device_id='$DEVICE_ID';"`
		if [[ -z "$ALT_IP_ID" ]]
			then
...
echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}Updating the $DEVICE_TABLE table for $IP"${RESET} >> $LOG
mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $LOCAL_DATABASE << EOF
UPDATE $DEVICE_TABLE SET updated=NOW(),dns_name='$DNS_NAME' WHERE id='$DEVICE_ID';
EOF
}

UPDATE_MAC ()
{
echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}Updating the mac addresses for devices without SNMP"${RESET} >> $LOG
DEVICE_IDS=
DEVICE_IDS=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT id FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE (mac IS NULL OR mac='') AND ping_check > (NOW() - INTERVAL 7 DAY) ORDER BY updated DESC LIMIT 50;"`
for DEVICE_ID in $DEVICE_IDS
	do
		IP=
		IP=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT ip FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE id='$DEVICE_ID';"`
		mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $LOCAL_DATABASE << EOF
UPDATE $DEVICE_TABLE set mac=(SELECT mac FROM $ARP_TABLE WHERE ip='$IP' AND mac!='ff:ff:ff:ff:ff:ff' ORDER BY updated DESC LIMIT 1) WHERE id='$DEVICE_ID';
EOF
done
mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $LOCAL_DATABASE << EOF
UPDATE $DEVICE_TABLE set mac=NULL WHERE mac='00:00:00:00:00:00';
UPDATE $DEVICE_TABLE set mac=NULL WHERE mac='ff:ff:ff:ff:ff:ff';
EOF
}

UPDATE_OUI ()
{
echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}Updating the oui for devices that haven't updated yet"${RESET} >> $LOG
DEVICE_IDS=
DEVICE_IDS=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT id FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE (oui IS NULL AND mac IS NOT NULL) AND ((ping_check > (NOW() - INTERVAL 7 DAY) AND updated < (NOW() - INTERVAL 1 DAY)) OR added > (NOW() - INTERVAL 1 DAY)) ORDER BY updated LIMIT 50;"`
for DEVICE_ID in $DEVICE_IDS
	do
		# echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}Updatine Device ID: $DEVICE_ID"${RESET} >> $LOG
		MAC=
		MAC=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT mac FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE id='$DEVICE_ID';"`
		GET_MAC_OUI
		# echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}OUI: $OUI"${RESET} >> $LOG
mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $LOCAL_DATABASE << EOF
UPDATE $DEVICE_TABLE set oui="$OUI" WHERE id='$DEVICE_ID';
EOF
done
}
...
if [[ -z "$IPS" ]]
	then
		echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${REDF}No IPs or Hostnames provided found devices without SNMP"${RESET} >> $LOG
fi
