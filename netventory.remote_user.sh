...
# WINDOWS_VPN_CONNECTIONS=$(winexe-static-2 --authentication-file=$WINEXE_CREDENTIALS //$SERVER_NAME "powershell.exe Get-RemoteAccessConnectionStatistics | FT -AutoSize" 2> /dev/null | grep --color=never "Vpn" | sed 's/\s\s*/;;/g' | sed 's/;;Vpn;;$//g' | tr [:upper:] [:lower:])
winexe-static-2 --authentication-file=$WINEXE_CREDENTIALS //$SERVER_NAME "powershell.exe Get-RemoteAccessConnectionStatistics | Select-Object -property ClientIPAddress,UserName,ConnectionDuration,ClientExternalAddress | FT -Autosize" 2> /dev/null | tr [:upper:] [:lower:] | sed '/^[[:space:]]*$/d' | sed '/\s\s*ddress/d' | sed '/clientipaddress/d' | sed 's/{//g' | sed 's/}//g' | sed 's/\s\s*/;;/g' | sed '/---/d' | sed 's/;;$//g' > $WINDOWS_VPN_FILE
WINDOWS_VPN_USERNAMES=$(cat $WINDOWS_VPN_FILE | egrep --color=never -o ';;[A-Za-z0-9\.\_-]+\\[A-Za-z0-9\.\_-]+;;' | sed 's/^;;.*\\//g' | sed 's/;;//g' | tr [:upper:] [:lower:] | sort --unique)
...
for USERNAME in $WINDOWS_VPN_USERNAMES
	do
		VPN_CONNECTION_ARRAY=($(cat $WINDOWS_VPN_FILE | grep -i --color=never "$USERNAME" | tail -n 1 | sed 's/;;/ /g'))
		echo "`date +%b\ %d\ %Y\ %H:%M:%S`: ==========" >> $LOG
		SERVER_NAME=$WINDOWS_VPN_SERVER
		REMOTE_IP=${VPN_CONNECTION_ARRAY[3]}
		IP=${VPN_CONNECTION_ARRAY[0]}
		DURATION=${VPN_CONNECTION_ARRAY[2]}
		CONVERT_SECONDS
		GET_ENDPOINT_ID
		GET_USER_ID
...
for USERNAME in $OPENVPN_USERNAMES
	do
		# VPN_CONNECTION=$(cat $EXPECT_LOG | grep -i --color=never "$USERNAME" | tail -n 1)
		VPN_CONNECTION_ARRAY=($(cat $EXPECT_LOG | grep -i --color=never "$USERNAME" | tail -n 1 | sed 's/;;/ /g'))
		echo "`date +%b\ %d\ %Y\ %H:%M:%S`: ==========" >> $LOG
		REMOTE_IP=${VPN_CONNECTION_ARRAY[7]}
		IP=${VPN_CONNECTION_ARRAY[8]}
		CONNECTION_TIME=${VPN_CONNECTION_ARRAY[2]}
		CONNECTION_TIME=$(echo "$CONNECTION_TIME" | sed 's/_MST//g' | sed 's/_/ /g')
		DURATION=${VPN_CONNECTION_ARRAY[3]}
		SERVER_NAME=${VPN_CONNECTION_ARRAY[0]}
...
for USERNAME in $NETMOTION_USERNAMES
	do
		VPN_CONNECTION_ARRAY=($(cat $NETMOTION_FILE | grep -i --color=never "$USERNAME" | tail -n 1 | sed 's/;;/ /g'))
		echo "`date +%b\ %d\ %Y\ %H:%M:%S`: ==========" >> $LOG
		REMOTE_IP=${VPN_CONNECTION_ARRAY[0]}
		SERVER_NAME=${VPN_CONNECTION_ARRAY[1]}
		SERVER_NAME=$(echo "$SERVER_NAME" | tr [:upper:] [:lower:])
		IP=${VPN_CONNECTION_ARRAY[5]}
		CONNECTION_TIME=${VPN_CONNECTION_ARRAY[3]}
...
# echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}SELECT id FROM $NETVENTORY_DATABASE.$REMOTE_USER_TABLE WHERE endpoint_id='$ENDPOINT_ID' AND server_name='$SERVER_NAME' AND domain='$DOMAIN' AND user_id='$USER_ID' AND ip='$IP' ORDER BY connection_time DESC LIMIT 1"${RESET} >> $LOG
VPN_USER_ID=
VPN_USER_ID=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT id FROM $NETVENTORY_DATABASE.$REMOTE_USER_TABLE WHERE endpoint_id='$ENDPOINT_ID' AND server_name='$SERVER_NAME' AND domain='$DOMAIN' AND user_id='$USER_ID' AND ip='$IP' ORDER BY connection_time DESC LIMIT 1;"`
if [[ -z "$VPN_USER_ID" ]]
	then
		echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}$IP does not exist in the database yet. Adding it"${RESET} >> $LOG
		ADD_VPN_USER_RECORD
...
mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $NETVENTORY_DATABASE << EOF
UPDATE $REMOTE_USER_TABLE SET endpoint_id='$ENDPOINT_ID',server_name='$SERVER_NAME',server_ip='$SERVER_IP',domain='$DOMAIN',user_id='$USER_ID',ip='$IP',duration='$DURATION',status='$STATUS',connection_type='$CONNECTION_TYPE',connection_time='$CONNECTION_TIME',remote_ip='$REMOTE_IP',connection_check=NOW(),updated=NOW() WHERE id='$VPN_USER_ID';
EOF
}
