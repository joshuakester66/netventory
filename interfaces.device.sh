#!/bin/bash
#Filename: netventory.interfaces.device.sh
#Description: 
...
SCRIPT_NAME="netventory.interfaces.device"
SCRIPT_CAT="netventory"

IPS=$1

mkdir -p $HOME/scripts/logs/$SCRIPT_CAT &> /dev/null
mkdir -p $HOME/scripts/tmp/$SCRIPT_NAME &> /dev/null
TODAY=`date +%Y-%m-%d`
NOW=`date +%Y-%m-%d\ %H:%M:%S`
LOG="$HOME/scripts/logs/$SCRIPT_CAT/$SCRIPT_NAME.log"
...
NUMBER_OF_IPS=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT 	COUNT(ip) FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE snmp_enabled='1' AND ping_check > (NOW() - INTERVAL 7 DAY) AND ip NOT LIKE '169.%.%.%' order by interface_check;"`
IPS=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT ip FROM $LOCAL_DATABASE.$DEVICE_TABLE WHERE snmp_enabled='1' AND ping_check > (NOW() - INTERVAL 7 DAY) AND ip NOT LIKE '169.%.%.%' order by interface_check LIMIT $DEVICES_PER_RUN;"`
}
...
if [[ -z "$INTERFACE_UPDATE" ]]
	then
		NEW_DEVICE=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT updated FROM $LOCAL_DATABASE.$INTERFACE_TABLE WHERE device_id='$DEVICE_ID';"`
		if [[ -z "$NEW_DEVICE" ]]
			then
				INTERFACE_UPDATE="1"
		fi
...
if [[ -z "$PORT_ALIAS" ]]
	then
		PORT_ALIAS=`mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --silent --skip-column-names -e "SELECT port_name FROM $LOCAL_DATABASE.$INTERFACE_TABLE WHERE device_id='$DEVICE_ID' and is_vlan='1' and port_name='$PORT_NAME';"`
fi
...
mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $LOCAL_DATABASE << EOF
INSERT INTO $INTERFACE_TABLE(device_id,port,port_admin,port_status,port_status_change,port_poe,module,port_name,port_alias,port_type,vlan,updated,added) VALUES('$DEVICE_ID','$PORT','$PORT_ADMIN','$PORT_STATUS','$PORT_STATUS_CHANGE','$PORT_POE','$MODULE','$PORT_NAME',"$PORT_ALIAS",'$PORT_TYPE','$VLAN',NOW(),NOW());
EOF
}
...
echo -e "`date +%b\ %d\ %Y\ %H:%M:%S`: ${CYANF}Updating the interface with ID $INTERFACE_ID"${RESET} >> $LOG
mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD $LOCAL_DATABASE << EOF
UPDATE $DEVICE_TABLE SET interface_check=NOW() WHERE id='$DEVICE_ID';
EOF
}
